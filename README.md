...
Ensure the VirusTotal API is working by running Mode 3B, and select option 2 from the menu. For more information, please visit: https://docs.virustotal.com/docs/api-overview.

Discord Webhook Setup: To receive notifications from the app in Discord, please follow these steps:
...
In the .env file, add this: WEBHOOK=yourdiscordwebhook and save.
Ensure the webhook is working by running Mode 3B, and select option 3 from the menu.
After verification is successful, you are done!
Language
...
Do NOT include --source2 or --ext. If you do, the program will not work. Instead follow step 3.

To check for the file in manifest.json, please use --mode "1B". Otherwise, to check for the file in manifest2.json, please use --mode "2B".

If you see something odd or suspicious, do NOT overwrite your manifests or delete them. Instead, manually check the file in question.

Improperly formatted JSON will result in the program throwing errors. It is wise to not manually modify the manifest files, and instead copy them and place them somewhere secure, before creating new manifests.
