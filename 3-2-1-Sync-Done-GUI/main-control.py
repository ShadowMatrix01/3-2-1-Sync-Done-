...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.dropdown = ctk.CTkOptionMenu(
                master=self,
                values=["Mode A1", "Mode A2", "Mode 1B", "Mode 2B", "Mode C", "Mode D1", "Mode D2"],
                command=self.mode_return
                )
        self.dropdown.grid(row=2, column=1, padx=20, pady=20, sticky="w")
        ...
        self.textbox.insert("0.0", "3--2-1 Sync Done! A Data Integrity Solution" 
                             ...
                             "\n[C]: Check if Local Notifications, Discord Webhook, Azure Blob Storage, and VirusTotal API are working as intended"
    ...
    def mode_return(self, value):
        ...
        if value == "Mode A1" or value == "Mode A2":
    ...
    def safe_destroy(self):
        ...
        try:
           ...
           pass
    def run(self):
      if app.argv.source == " " and not (app.argv.mode == "Mode C" or app.argv.mode =="D1" or app.argv.mode == "Mode D2"):
      ...
      if app.argv.mode == "Mode A1":
         ...
         download_blob(app.argv.ext, manifest_target, None)
         print(f"Program finished at {datetime.now()}")
         self.after(0, self.safe_destroy)
         ...
         validate()
         manifest_target = load_manifest("manifest_cloud.json")
         schedule.every().day.at(time_task, timezone(timezone_task)).do(download_blob, app.argv.ext, manifest_target, check_for_virus)
         print(f"Program finished at {datetime.now()}")
         while True:
            schedule.run_pending()
            time.sleep(1)  
         ...
         if not valid:
               print("Error: Invalid format for directory. Valid example (Windows) C:/Users/Downloads or C:\\Users\\Downloads")
               exit()
         mover(app.argv.source, "manifest")
         print(f"Program finished at {datetime.now()}")
         self.after(0, self.safe_destroy)
         ...
         if not valid:
               print("Error: Invalid format for directory. Valid example (Windows) C:/Users/Downloads or C:\\Users\\Downloads")
               exit()
         mover(app.argv.source, "manifest2")
         print(f"Program finished at {datetime.now()}")
         self.after(0, self.safe_destroy)
         ...
         if not app.argv.source2:
            print(f"{app.argv.mode} is not a valid mode. Please try again.")
         else:
